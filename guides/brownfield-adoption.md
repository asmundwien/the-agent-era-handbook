...
> **Core chapters referenced:** Handbook 3 (SDD), 4 (Operations), 6 (Failure Modes).

## Most Adoption Is Brownfield

You have an existing codebase, existing conventions (or lack thereof), and existing technical debt. You cannot start from scratch. You should not try. This guide covers how to introduce spec-driven development incrementally without rewriting anything.
...
Apply SDD (Handbook 3) to the next feature you would build anyway. Write a spec for that feature. Let the agent implement from the spec. Review the output against the acceptance criteria.
...
Do not attempt to retroactively write specs for existing functionality. That is archaeology, not development.
...
- **Week 3:** Run your first spec-driven feature end-to-end. Spec, design, tasks, implementation, review.
...
**Existing test coverage matters more than ever.** Tests are the primary feedback signal for agent output quality. If a module has no tests, agent changes to that module are unverifiable. Prioritize test coverage on modules you plan to hand to agents.
...
The first spec-driven feature will take longer than just coding it yourself. The second will be faster. By the tenth, the overhead is negligible and the consistency gains are compounding.
